...
test('MapView has no markers if location array is empty', () => {
  const { container } = render(<MapView viewport={mockViewport} locationArray={[]} />);
  expect(container.querySelector('.map-view__marker')).toBeNull();
});

test('MapView markers count equals location array length', () => {
  const { container } = render(<MapView viewport={mockViewport} locationArray={mockLocationArray} />);
...
test('MapView selected marker has different class than others', () => {
  const { container } = render(
    <MapView viewport={mockViewport} locationArray={mockLocationArray} selectedLocation={mockSelectedLocation} />
  );
